...
def test_nao_serve_arquivo_fora_do_pdf_dir(tmp_path, monkeypatch):
    pdf_dir = tmp_path / "dentro"; pdf_dir.mkdir()
    fora_dir = tmp_path / "fora"; fora_dir.mkdir()
    monkeypatch.setenv("MYLIC_PDF_DIR", str(pdf_dir))
    fora = fora_dir / "segredo.pdf"; fora.write_bytes(b"%PDF-1.4 fora")
    from datetime import date, datetime, timezone
    from decimal import Decimal
    from app.db.engine import make_session
    from app.db.models import EditalRow, ArquivoEditalRow
    S = make_session(os.environ["MYLIC_DATABASE_URL"])
    with S() as s:
        row = EditalRow(fonte="pncp", chave_natural="arq2", hash_conteudo="h", objeto="clipping",
                        orgao_nome="Org", orgao_cnpj="0", uf="AM", modalidade="Pregao",
                        valor_estimado=Decimal("1"), data_publicacao=date(2026, 9, 1),
                        score_relevancia=0.9, status="novo", url_origem="x",
                        capturado_em=datetime.now(timezone.utc))
        s.add(row); s.flush()
        a = ArquivoEditalRow(edital_id=row.id, tipo="edital", caminho_local=str(fora),
                             hash="h", baixado_em=datetime.now(timezone.utc))
        s.add(a); s.commit(); eid, aid = row.id, a.id
    from app.api.main import app
    c = TestClient(app)
    assert c.get(f"/editais/{eid}/arquivo/{aid}").status_code == 404
